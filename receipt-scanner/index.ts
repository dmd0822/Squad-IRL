...
async function main(): Promise<void> {
  ...
  if (receipts.length === 0) {
    console.log(`${C.yellow}  No receipt files found (.txt, .md, .csv). Add some and try again.${C.reset}`);
    return;
  }
  ...
  console.log(`${C.dim}     • Add OCR integration to process scanned paper receipts${C.reset}`);
  console.log();
