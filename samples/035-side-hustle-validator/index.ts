/**
 * Side Hustle Validator — Squad SDK Sample
 *
 * Evaluate side business ideas for market fit, competition, and profitability
 *
 * Audience: Everyone
...
const AGENTS = [
  { name: 'MarketResearcher', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Realist', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
...
async function main(): Promise<void> {
  console.log('🚀 Side Hustle Validator\n');
  console.log('👥 Audience: Everyone');
  console.log('📋 Problem: Evaluate side business ideas for market fit, competition, and profitability\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[MarketResearcher output v${iteration}]\n${writerInput.slice(0, 500)}`;
