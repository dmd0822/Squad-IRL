/**
 * Job Application Tailor — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Recruiter', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Matcher', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'Wordsmith', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Job Application Tailor\n');
  console.log('👥 Audience: 🎓 Students/Everyone');
  console.log('📋 Problem: Customize resume and cover letter for each job posting\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Recruiter output v${iteration}]\n${writerInput.slice(0, 500)}`;
