/**
 * Recipe Scaler — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Chef', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'MathWiz', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'TimingExpert', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Recipe Scaler\n');
  console.log('👥 Audience: 🍳 Foodies');
  console.log('📋 Problem: Scale recipes up/down and adjust cooking times/temps accordingly\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Chef output v${iteration}]\n${writerInput.slice(0, 500)}`;
