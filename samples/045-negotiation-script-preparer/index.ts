/**
 * Negotiation Script Preparer — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Negotiator', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
...
async function main(): Promise<void> {
  console.log('🚀 Negotiation Script Preparer\n');
  console.log('👥 Audience: Everyone');
  console.log('📋 Problem: Prepare for salary/price negotiations with scripts and counterargument prep\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Negotiator output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Negotiation Script Preparer`,
    `- **Audience:** Everyone`,
