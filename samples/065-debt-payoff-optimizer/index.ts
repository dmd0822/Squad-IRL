/**
 * Debt Payoff Optimizer — Squad SDK Sample
 *
 * Create optimized debt payoff plans using avalanche/snowball methods
 *
 * Audience: 💰 Finance
...
const AGENTS = [
  { name: 'DebtCoach', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'MathBot', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'MotivationTracker', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Debt Payoff Optimizer\n');
  console.log('👥 Audience: 💰 Finance');
  console.log('📋 Problem: Create optimized debt payoff plans using avalanche/snowball methods\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[DebtCoach output v${iteration}]\n${writerInput.slice(0, 500)}`;
