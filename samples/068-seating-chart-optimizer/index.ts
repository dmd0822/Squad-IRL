/**
 * Seating Chart Optimizer — Squad SDK Sample
 *
 * Create wedding/event seating charts that minimize drama and maximize joy
 *
...
const AGENTS = [
  { name: 'SocialDynamicsExpert', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'PuzzleSolver', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'DiplomatBot', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Seating Chart Optimizer\n');
  console.log('👥 Audience: 👰 Event Planners');
  console.log('📋 Problem: Create wedding/event seating charts that minimize drama and maximize joy\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    // ── Write Phase ──
    console.log('\n✏️  SocialDynamicsExpert working...');
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[SocialDynamicsExpert output v${iteration}]\n${writerInput.slice(0, 500)}`;
    saveOutput(`draft-v${iteration}.md`, currentOutput);
    console.log('   ✅ Draft saved');

    // ── Grade Phase ──
    console.log('\n📊 PuzzleSolver evaluating...');
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Seating Chart Optimizer`,
    `- **Audience:** 👰 Event Planners`,
