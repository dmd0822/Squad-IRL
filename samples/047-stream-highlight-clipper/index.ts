/**
 * Stream Highlight Clipper — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'ClipSpotter', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Editor', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'TitleWriter', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Stream Highlight Clipper\n');
  console.log('👥 Audience: 🎮 Gamers/Streamers');
  console.log('📋 Problem: Identify and caption highlight-worthy moments from gaming streams\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[ClipSpotter output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Stream Highlight Clipper`,
    `- **Audience:** 🎮 Gamers/Streamers`,
