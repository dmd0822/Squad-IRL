...
const AGENTS = [
  { name: 'Quizmaster', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'BloomsTaxonomyExpert', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'AnswerKeyMaker', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  ...
  console.log('📋 Problem: Generate quizzes from lesson content with varied question types\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Quizmaster output v${iteration}]\n${writerInput.slice(0, 500)}`;
    saveOutput(`draft-v${iteration}.md`, currentOutput);
    console.log('   ✅ Draft saved');

    // ── Grade Phase ──
    console.log('\n📊 BloomsTaxonomyExpert evaluating...');
