/**
 * Kids Allowance Manager — Squad SDK Sample
 *
 * Create allowance systems that teach financial responsibility
 *
 * Audience: 👩‍👧‍👦 Parents
...
const AGENTS = [
  { name: 'FinancialEducator', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'FairnessJudge', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'GoalSetter', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Kids Allowance Manager\n');
  console.log('👥 Audience: 👩‍👧‍👦 Parents');
  console.log('📋 Problem: Create allowance systems that teach financial responsibility\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[FinancialEducator output v${iteration}]\n${writerInput.slice(0, 500)}`;
    saveOutput(`draft-v${iteration}.md`, currentOutput);
    console.log('   ✅ Draft saved');

    // ── Grade Phase ──
    console.log('\n📊 FairnessJudge evaluating...');
