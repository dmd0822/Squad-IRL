/**
 * Study Guide Creator — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Professor', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Quizzer', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'MemoryExpert', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Study Guide Creator\n');
  console.log('👥 Audience: 🎓 Students');
  console.log('📋 Problem: Turn lecture notes and textbook chapters into comprehensive study guides\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Professor output v${iteration}]\n${writerInput.slice(0, 500)}`;
