/**
 * Customer Review Responder — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'CustomerService', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'ToneManager', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'ProblemSolver', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Customer Review Responder\n');
  console.log('👥 Audience: 🏪 Small Business');
  console.log('📋 Problem: Draft thoughtful responses to customer reviews (positive and negative)\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[CustomerService output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Customer Review Responder`,
    `- **Audience:** 🏪 Small Business`,
