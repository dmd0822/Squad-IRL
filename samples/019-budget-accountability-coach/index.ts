/**
 * Budget Accountability Coach — Squad SDK Sample
 *
 * Weekly spending review with tough love and actionable advice
 *
 * Audience: 💰 Finance/Everyone
...
const AGENTS = [
  { name: 'Coach', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Realist', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'Motivator', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Budget Accountability Coach\n');
  console.log('👥 Audience: 💰 Finance/Everyone');
  console.log('📋 Problem: Weekly spending review with tough love and actionable advice\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Coach output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Budget Accountability Coach`,
    `- **Audience:** 💰 Finance/Everyone`,
