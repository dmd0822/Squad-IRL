/**
 * Wine Pairing Suggester — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Sommelier', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Educator', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'BudgetAdvisor', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Wine Pairing Suggester\n');
  console.log('👥 Audience: 🍳 Foodies');
  console.log('📋 Problem: Suggest wine pairings for meals with educational tasting notes\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Sommelier output v${iteration}]\n${writerInput.slice(0, 500)}`;
