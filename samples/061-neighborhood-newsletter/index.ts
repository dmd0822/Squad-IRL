/**
 * Neighborhood Newsletter — Squad SDK Sample
 *
 * Create community newsletters from local happenings and announcements
 *
...
const AGENTS = [
  { name: 'Reporter', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Editor', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'CommunityVoice', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Neighborhood Newsletter\n');
  console.log('👥 Audience: 🏘️ Community');
  console.log('📋 Problem: Create community newsletters from local happenings and announcements\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Reporter output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Neighborhood Newsletter`,
    `- **Audience:** 🏘️ Community`,
