...
const AGENTS = [
  { name: 'Statistician', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Interpreter', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'DecisionAdvisor', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Ab Test Analyzer\n');
  console.log('👥 Audience: 📊 Data/Business');
  console.log('📋 Problem: Analyze A/B test results and recommend decisions with statistical rigor\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Statistician output v${iteration}]\n${writerInput.slice(0, 500)}`;
