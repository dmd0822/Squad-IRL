...
const AGENTS = [
  { name: 'SpecialEdExpert', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'ComplianceChecker', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'GoalCrafter', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Iep Goal Writer\n');
  console.log('👥 Audience: 🏫 Teachers/SPED');
  console.log('📋 Problem: Write measurable IEP goals that meet compliance and support student growth\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[SpecialEdExpert output v${iteration}]\n${writerInput.slice(0, 500)}`;
    saveOutput(`draft-v${iteration}.md`, currentOutput);
    console.log('   ✅ Draft saved');

    // ── Grade Phase ──
    console.log('\n📊 ComplianceChecker evaluating...');
