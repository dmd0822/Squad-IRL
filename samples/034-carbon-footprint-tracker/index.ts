/**
 * Carbon Footprint Tracker — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Calculator', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Researcher', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'ActionPlanner', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Carbon Footprint Tracker\n');
  console.log('👥 Audience: 🌱 Environmental');
  console.log('📋 Problem: Calculate personal carbon footprint and suggest highest-impact reductions\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Calculator output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Carbon Footprint Tracker`,
    `- **Audience:** 🌱 Environmental`,
