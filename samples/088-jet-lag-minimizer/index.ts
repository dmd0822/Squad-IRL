/**
 * Jet Lag Minimizer — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'SleepScientist', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Scheduler', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'TimezoneCalculator', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Jet Lag Minimizer\n');
  console.log('👥 Audience: ✈️ Travelers');
  console.log('📋 Problem: Create pre-trip schedules to minimize jet lag with sleep/light exposure timing\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[SleepScientist output v${iteration}]\n${writerInput.slice(0, 500)}`;
