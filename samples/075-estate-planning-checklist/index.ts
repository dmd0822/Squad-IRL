/**
 * Estate Planning Checklist — Squad SDK Sample
 *
 * Comprehensive estate planning task list with priority and timeline
 *
...
const AGENTS = [
  { name: 'EstatePlanner', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Organizer', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'DocumentTracker', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Estate Planning Checklist\n');
  console.log('👥 Audience: 👵 Seniors/Adults');
  console.log('📋 Problem: Comprehensive estate planning task list with priority and timeline\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[EstatePlanner output v${iteration}]\n${writerInput.slice(0, 500)}`;
