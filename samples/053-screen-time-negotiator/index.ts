/**
 * Screen Time Negotiator — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Mediator', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'FairnessJudge', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'ContractWriter', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Screen Time Negotiator\n');
  console.log('👥 Audience: 👩‍👧‍👦 Parents');
  console.log('📋 Problem: Create fair screen time agreements with kids that both sides accept\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Mediator output v${iteration}]\n${writerInput.slice(0, 500)}`;
