/**
 * Scholarship Finder — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Matcher', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Researcher', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'EssayCoach', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Scholarship Finder\n');
  console.log('👥 Audience: 🎓 Students');
  console.log('📋 Problem: Match students to scholarship opportunities and draft application essays\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Matcher output v${iteration}]\n${writerInput.slice(0, 500)}`;
