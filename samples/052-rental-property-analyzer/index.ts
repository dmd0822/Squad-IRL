/**
 * Rental Property Analyzer — Squad SDK Sample
 *
 * Evaluate rental properties for cash flow, ROI, and hidden costs
 *
 * Audience: 💰 Finance/Investors
...
const AGENTS = [
  { name: 'RealEstateAnalyst', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'InspectorBot', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'NumbersCruncher', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Rental Property Analyzer\n');
  console.log('👥 Audience: 💰 Finance/Investors');
  console.log('📋 Problem: Evaluate rental properties for cash flow, ROI, and hidden costs\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[RealEstateAnalyst output v${iteration}]\n${writerInput.slice(0, 500)}`;
    saveOutput(`draft-v${iteration}.md`, currentOutput);
    console.log('   ✅ Draft saved');

    // ── Grade Phase ──
    console.log('\n📊 InspectorBot evaluating...');
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Rental Property Analyzer`,
    `- **Audience:** 💰 Finance/Investors`,
