/**
 * Lesson Plan Generator — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'Educator', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
...
async function main(): Promise<void> {
  console.log('🚀 Lesson Plan Generator\n');
  console.log('👥 Audience: 🏫 Teachers');
  console.log('📋 Problem: Create standards-aligned lesson plans with activities and assessments\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[Educator output v${iteration}]\n${writerInput.slice(0, 500)}`;
