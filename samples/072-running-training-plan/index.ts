/**
 * Running Training Plan — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'RunCoach', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'PhysioGuard', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'PaceCalculator', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Running Training Plan\n');
  console.log('👥 Audience: 🏃 Fitness');
  console.log('📋 Problem: Personalized marathon/race training plans with injury prevention\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[RunCoach output v${iteration}]\n${writerInput.slice(0, 500)}`;
