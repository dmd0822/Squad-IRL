/**
 * Supplement Stack Optimizer — Squad SDK Sample
 *
 * Research and optimize supplement regimens for goals and safety
 *
 * Audience: 🏃 Fitness/Health
...
const AGENTS = [
  { name: 'NutritionistBot', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'ResearcherBot', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'InteractionChecker', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Supplement Stack Optimizer\n');
  console.log('👥 Audience: 🏃 Fitness/Health');
  console.log('📋 Problem: Research and optimize supplement regimens for goals and safety\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[NutritionistBot output v${iteration}]\n${writerInput.slice(0, 500)}`;
    saveOutput(`draft-v${iteration}.md`, currentOutput);
    console.log('   ✅ Draft saved');

    // ── Grade Phase ──
    console.log('\n📊 ResearcherBot evaluating...');
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Supplement Stack Optimizer`,
    `- **Audience:** 🏃 Fitness/Health`,
