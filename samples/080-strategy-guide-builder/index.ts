/**
 * Strategy Guide Builder — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'ProGamer', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'Researcher', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'GuideWriter', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Strategy Guide Builder\n');
  console.log('👥 Audience: 🎮 Gamers');
  console.log('📋 Problem: Create comprehensive game strategy guides with tips and tactics\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[ProGamer output v${iteration}]\n${writerInput.slice(0, 500)}`;
