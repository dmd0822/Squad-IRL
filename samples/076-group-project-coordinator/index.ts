/**
 * Group Project Coordinator — Squad SDK Sample
 *
...
const AGENTS = [
  { name: 'ProjectManager', role: 'developer' as AgentRole, promptFile: 'agents/writer.md' },
  { name: 'FairnessJudge', role: 'tester' as AgentRole, promptFile: 'agents/grader.md' },
  { name: 'CommunicationHub', role: 'developer' as AgentRole, promptFile: 'agents/updater.md' },
];
...
async function main(): Promise<void> {
  console.log('🚀 Group Project Coordinator\n');
  console.log('👥 Audience: 🎓 Students');
  console.log('📋 Problem: Manage group projects with task delegation and deadline tracking\n');
  ...
  while (grade < GRADE_THRESHOLD && iteration < MAX_ITERATIONS) {
    ...
    const writerInput = iteration === 1
      ...
      : `SPEC:\n${spec}\n\nPREVIOUS OUTPUT:\n${currentOutput}\n\nGRADE: ${grade}/100\n\nImprove the output.`;
    currentOutput = `[ProjectManager output v${iteration}]\n${writerInput.slice(0, 500)}`;
  ...
  saveOutput('final-report.md', [
    '# Review Loop Report',
    '',
    `- **Sample:** Group Project Coordinator`,
    `- **Audience:** 🎓 Students`,
